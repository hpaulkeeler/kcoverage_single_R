...
funIn<-function(betaConst,n,x){
  ...
  In<-(2/betaConst)^(n-1)/C^n; #eq. (14) in [1]
    ...
    #u=log(1./t).^(1/2)  
    ft <- function(t) (-log(t))^(n-1)*exp(-(-log(t))^(betaConst/2)*x*((gamma(1-2/betaConst))^(-betaConst/2)))/2;    
    In <- 2^n/betaConst^(n-1)*(integrate(ft, lower=0, upper=1)$val)/C^n/factorial(n-1);
    
    #ft <- function(arg, weight) {
    #  t <- arg[1]
    #  ff <- (-log(t))^(n-1)*exp(-(-log(t))^(betaConst/2)*x*((gamma(1-2/betaConst))^(-betaConst/2)))/2;;
    #  return(ff)
    #} # end integrand
    
    #cuhre(1,1,ft,lower=rep(0,1),upper=rep(1,1),flags= list(verbose=0, final=1))$value;
    #In <- 2^n/betaConst^(n-1)*(cuhre(1,1,ft,lower=rep(0,1),upper=rep(1,1),flags= list(verbose=0, final=1))$value)/C^n/factorial(n-1);
  }
  funIn<-In;
}